import Button             from '@/components/ui/MovingBorder';
import SectionHead        from '@/components/ui/SectionHead';
import { workExperience } from '@/data';
import React              from 'react';

const Experience = () => {
	return (
		<section id='testimonials' className='py-20'>
			<SectionHead
				text='My work experience'
				accent={{ 'text-purple' : [ 1, 2 ] }}
			/>

			<div className='w-full mt-12 grid lg:grid-cols-4 grid-cols-1 gap-10'>
				{workExperience.map( ( { id, className, title, thumbnail, desc }, idx ) => (
					<Button
						id={id}
						key={idx}
						borderRadius='1.75rem'
						className='flex-1 text-white border-slate-800'
						duration={Math.floor( Math.random() * 10000 ) + 10000}
					>
						<div className='flex lf:flex-row flex-col lg:items-center p-3 py-6 md:p-5 lg:p-10 gap-2'>
							<img src={thumbnail} alt={title} className='lg:w-32 md:w-20 w-16' />
							<div className='lg:ms-5'>
								<h1 className='text-start text-xl md:text-2xl font-bold'>{title}</h1>
								<p className='text-start text-white-100 mt-3 font-semibold'>{desc}</p>
							</div>
						</div>
					</Button>
				) )}
			</div>
		</section>
	);
};

export default Experience;